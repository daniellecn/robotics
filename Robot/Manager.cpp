...
void Manager::run() {
	...
	while(count < 400) {
		...
		while(!(_curr->stopCondition()) && count < 400) {
			_robot->read();
			count++;
			_curr->action();
			_robot->calcDeltas();
			cout << "robot real: [" << _robot->getXPos() << "," << _robot->getYPos() << "," << _robot->getYaw() << "]" << endl;
			cout << "robot pos: [" << _robot->getCurrPos().x << "," << _robot->getCurrPos().y << "," << _robot->getCurrPos().yaw << "]" << endl;
